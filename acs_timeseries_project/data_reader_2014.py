...
Differences from 2019: all Excel files are .xls, not .xlsx, input folders have
different names and directory structures, the sequence file had an empty space instead
of null values which caused the section of the program that generates the sequences
dictionary to fail; corrected this by adding lines to populate nulls, variable
IDs in the keeper file did not have dashes between the table and variable,
wrote a function to insert them.
...
datafold1=os.path.join(infold,'tab4','sumfile','prod','2010thru2014','group1')
datafold2=os.path.join(infold,'tab4','sumfile','prod','2010thru2014','group2')
tempfolder=os.path.join(infold,'{}_5yr_Summary_FileTemplates','seq').format(year)
csvfolder=os.path.join(outfold,'csv_todb')

varskeep_file=os.path.join(infold,'ACS{}_Table_Shells.xlsx').format(year)
seq_file=os.path.join(infold,'ACS_5yr_Seq_Table_Number_Lookup.xls')
...
nan_value = float("NaN")
seq_df.replace(" ", nan_value, inplace=True)
...
for idx, row in tabseq_df.iterrows():
    if row['Table ID'] not in seq:
        seq[row['Table ID']]=[str(row['Sequence Number'])] # First time, create value list
        if row['Table ID'] in tables:
            tabnames[row['Table ID']]=row['Table Title']
    else:
        seq[row['Table ID']].append(str(row['Sequence Number'])) # Just append to value list
...
for k,v in tables.items(): # k is table, v list of sequences
    ...
    for s in slist:
        tempfile=os.path.join(tempfolder,'Seq'+s+'.xls')
